...
class Employee(models.Model):
    ...
    def get_user_employee_details(self):
        ...
        # Ensure 'id', 'birthday', and 'joining_date' are retrieved
        employee = self.env['hr.employee'].sudo().search_read(
            [('user_id', '=', uid)], ['id', 'birthday', 'joining_date'], limit=1)

        if not employee:
            return False

        emp_id = employee[0].get('id')
        if not emp_id:
            return False  # Ensure ID exists

        # Get broad_factor
        self.env.cr.execute("SELECT broad_factor FROM hr_employee_broad_factor WHERE id = %s", (emp_id,))
        result = self.env.cr.dictfetchall()
        broad_factor = result[0]['broad_factor'] if result else 0

        # Count leaves to approve
        leaves_to_approve = self.env['hr.leave'].sudo().search_count(
            [('state', 'in', ['confirm', 'validate1'])]
        )
        ...
        query = """
            SELECT COUNT(id) FROM hr_leave 
            WHERE (hr_leave.date_from::DATE, hr_leave.date_to::DATE) OVERLAPS (%s, %s)
            AND state = 'validate'
        """
        self.env.cr.execute(query, (today, today))
        leaves_today = self.env.cr.fetchone()[0]

        # Count leaves for the current month
        first_day = date.today().replace(day=1)
        last_day = (date.today() + relativedelta(months=1, day=1)) - timedelta(days=1)
        self.env.cr.execute(query, (first_day, last_day))
        leaves_this_month = self.env.cr.fetchone()[0]
        ...
        birthday = employee[0].get('birthday')
        age = relativedelta(datetime.today(), birthday).years if birthday else False

        joining_date = employee[0].get('joining_date')
        if joining_date:
            diff = relativedelta(datetime.today(), joining_date)
        ...
        data = {
            ...
            'emp_timesheets': timesheet_count,
            'job_applications': job_applications,
            'timesheet_view_id': timesheet_view_id.id if timesheet_view_id else False,
            'experience': experience,
            'age': age
        }
