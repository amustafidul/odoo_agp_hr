...
class NeracaSummaryController(http.Controller):
    @http.route([
        '/neraca_summary/export/<model("neraca.summary.mb.wizard"):wizard_id>',
    ], type='http', auth='user', csrf=False)
    def export_neraca_summary(self, wizard_id=False, **args):
        response = request.make_response(
                    None,
                    headers=[
                        ('Content-Type', 'application/vnd.ms-excel'),
                        ('Content-Disposition', content_disposition('Laporan Neraca Summary.xlsx'))
                    ]
                )
        ...
        row += 1
        sheet.write(row - 1, col, 'LAPORAN POSISI KEUANGAN KONSOLIDASIAN', title_style_left)
        sheet.write(row - 1, col + 4, 'CONSOLIDATED STATEMENT OF FINANCIAL POSITION', title_style_right)
        row += 1
        
        sheet.write(row - 1, col, f'Per 31 Desember {str(current_year)}', title_style_left)
        sheet.write(row - 1, col + 4, f'As of December 31, {str(current_year)}', title_style_right)
        ...
        # account_lines = self.get_account_lines(wizard_id)
