...
class CashflowController(http.Controller):
    @http.route([
        '/cashflow/export/<model("cashflow.mb.wizard"):wizard_id>',
    ], type='http', auth='user', csrf=False)
    def export_cashflow(self, wizard_id=False, **args):
        response = request.make_response(
            None,
            headers=[
                ('Content-Type', 'application/vnd.ms-excel'),
                ('Content-Disposition', content_disposition('Laporan Cashflow.xlsx'))
            ]
        )
        ...
        row += 1
        sheet.write(row - 1, col, 'LAPORAN ARUS KAS KONSOLIDASIAN', title_style_left)
        sheet.write(row - 1, col + 4, 'CONSOLIDATED STATEMENT OF CASH FLOW', title_style_right)
        row += 1

        sheet.write(row - 1, col, f'Untuk Tahun yang Berakhir 31 Desember {str(previous_year)}', title_style_left)
