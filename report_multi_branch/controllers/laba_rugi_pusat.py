...
class LabaRugiPusatController(http.Controller):
    @http.route([
        '/laba_rugi_pusat/export/<model("laba.rugi.mb.wizard"):wizard_id>',
    ], type='http', auth='user', csrf=False)
    def export_laba_rugi_pusat(self, wizard_id=False, **args):
        response = request.make_response(
                    None,
                    headers=[
                        ('Content-Type', 'application/vnd.ms-excel'),
                        ('Content-Disposition', content_disposition('LABA RUGI PUSAT.xlsx'))
                    ]
                )
        ...
        row = 1
        sheet.write(f'A{row}', 'LABA RUGI PUSAT', title_style)
        row += 1
