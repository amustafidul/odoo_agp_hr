...
class AgingAPController(http.Controller):
    @http.route([
        '/aging_ap/export/<model("aging.ap.wizard"):wizard_id>',
    ], type='http', auth='user', csrf=False)
    def export_aging_ap(self, wizard_id=False, **args):
        res = {}
        mapping = [
            ('emkl', 'EMKL'),
        ...
        sheet.merge_range(f'A{row}:I{row}', f'AGING HUTANG ASSIST {jenis_kegiatan}', title_style)
        row += 1
        periode = tools.ustr(
        ...
        sheet.merge_range(f'B{row}:B{row + 1}', 'KODE KREDITUR', header_style_border)
        sheet.merge_range(f'C{row}:C{row + 1}', 'NAMA KREDITUR', header_style_border)
