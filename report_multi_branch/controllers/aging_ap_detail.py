...
class AgingAPDetailController(http.Controller):
    @http.route([
        '/aging_ap_detail/export/<model("aging.ap.detail.wizard"):wizard_id>',
    ], type='http', auth='user', csrf=False)
    def export_aging_ap_detail(self, wizard_id=False, **args):
        res = {}
        mapping = [
            ('emkl', 'EMKL'),
        ...
        response = request.make_response(
                    ...
                    headers=[
                        ...
                        ('Content-Disposition', content_disposition(f'AGING HUTANG {jenis_kegiatan}.xlsx'))
                    ]
                )
        ...
        sheet.merge_range(f'A{row}:N{row}', f'AGING HUTANG ASSIST {jenis_kegiatan}', title_style)
        row += 1
        periode = tools.ustr(
