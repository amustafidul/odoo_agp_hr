...
class DebitNote(models.Model):
    _name = 'report.report_multi_branch.debit_note'
    ...
    account_line_ids = fields.One2many('report.debit_note.line', 'debit_note_id', string='Account Lines')
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        return super(DebitNote, self).search(args, offset, limit, order, count)
    ...
    @api.depends('account_line_ids.amount')
    def _compute_totals(self):
        for record in self:
            total_amount = sum(line.amount for line in record.account_line_ids)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):            
            ...
            vals['name'] = f'DN{sequence_code}/{department_code}/{branch_code}/{roman_month}/{year}'
    ...
    def print_debit_note(self):
        return self.env.ref('report_multi_branch.report_debit_note').report_action(self)
...
class DebitNoteLine(models.Model):
    _name = 'report.debit_note.line'
    _description = 'Debit Note Line'

    debit_note_id = fields.Many2one('report.report_multi_branch.debit_note', string='Debit Note Reference', required=True, ondelete='cascade')   
