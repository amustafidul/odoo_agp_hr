...
class AgingARDetailWizard(models.TransientModel):
    _name = 'aging.ar.detail.wizard'
    ...
    def action_view(self):
        ...
        for partner in partner_id:
            ...
            for move in move_partner:
                # Initialize the aging columns for each move
                total1 = total2 = total3 = total4 = total5 = 0.0

                age_id_days = int((self.periode - move.date).days)
                
                # Find relevant payments up to the selected period
        ...
        return self.env.ref('report_multi_branch.action_report_aging_ar_detail').report_action(self, data=data)
        
    
    def action_xls(self):
        return {
            'type': 'ir.actions.act_url',
            'url': '/aging_ar_detail/export/%s' % (self.id),
            'target': 'new',
        }
