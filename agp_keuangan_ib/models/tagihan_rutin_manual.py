...
class TagihanRutinManual(models.Model):
    _name = 'tagihan.rutin.manual'
    _description = 'Tagihan Rutin Manual'
    _inherit = ['portal.mixin', 'mail.thread', 'mail.activity.mixin', 'utm.mixin']

    name = fields.Char(string="Deskripsi", tracking=True)
    # nominal_pinjaman = fields.Float(string="Nominal Pinjaman", required=True, tracking=True)
    ...
    angsuran_line_manual_ids = fields.One2many('tagihan.rutin.manual.line', 'tagihan_rutin_manual_id', string="Detail Angsuran")
    ...
    total_nominal_angsuran = fields.Float(string='Total Nominal Angsuran', compute='_compute_totals', store=True)
    ...
    # @api.depends('jenis_perhitungan', 'shl_id')
    # def _compute_nominal_pinjaman(self):
    #     for record in self:
    #         if record.jenis_perhitungan == 'otomatis':
    #             record.nominal_pinjaman = record.shl_id.nominal_perjanjian or 0.0
    #         else:
    #             record.nominal_pinjaman = record.nominal_pinjaman  # Manual input
    
    
    # @api.onchange('jenis_perhitungan')
    # def _onchange_jenis_perhitungan(self):
    #     if self.jenis_perhitungan == 'manual':
    #         self.shl_id = False

    
    @api.depends('angsuran_line_manual_ids.sisa_pokok', 
                 'angsuran_line_manual_ids.nominal_pokok', 
                 'angsuran_line_manual_ids.nominal_bunga', 
                 'angsuran_line_manual_ids.nominal_pembayaran')

    def _compute_totals(self):
        for record in self:
            record.total_sisa_pokok = sum(line.sisa_pokok for line in record.angsuran_line_manual_ids)
            record.total_nominal_pokok = sum(line.nominal_pokok for line in record.angsuran_line_manual_ids)
            record.total_nominal_bunga = sum(line.nominal_bunga for line in record.angsuran_line_manual_ids)
            record.total_nominal_angsuran = sum(line.nominal_cicilan for line in record.angsuran_line_manual_ids)  # Asumsi Nominal Angsuran = Nominal Pembayaran
            record.total_nominal_pembayaran = sum(line.nominal_pembayaran for line in record.angsuran_line_manual_ids)
    ...
    def generate_angsuran(self):
        ...
        self.angsuran_line_manual_ids = [(5, 0, 0)]  # Hapus angsuran yang ada
        ...
        if self.nominal_pinjaman > 0 and self.cicilan > 0:
            ...
            for bulan in range(1, jumlah_cicilan + 1):
                ...
                self.angsuran_line_manual_ids.create({
                    'tagihan_rutin_manual_id': self.id,
                    'periode': periode,
    ...
    def export_to_excel(self):
        ...
        row = 1
        for line in self.angsuran_line_manual_ids:
...
class TagihanRutinLine(models.Model):
    _name = 'tagihan.rutin.manual.line'
    _description = 'Tagihan Rutin Manual Line'

    tagihan_rutin_manual_id = fields.Many2one('tagihan.rutin.manual', string='Tagihan Rutin')
    periode = fields.Char(string='Periode (Bulan Tahun)')
    nominal_pokok = fields.Float(string='Nominal Pokok', store=True)
    sisa_pokok = fields.Float(string='Sisa Pokok', store=True)
    nominal_bunga = fields.Float(string='Nominal Bunga', store=True)
        ...
        for line in self:
            ...
            if line.id:
                next_line = self.search([
                    ('tagihan_rutin_manual_id', '=', line.tagihan_rutin_manual_id.id),
