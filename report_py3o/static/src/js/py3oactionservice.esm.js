    ...
    .add("py3o_handler", async function (action, options, env) {
        if (action.report_type === "py3o") {
            ...
            try {
                ...
                // Andromeda logics to forcely streams the data to new tab instead of download
                let reportUrl = `/report/py3o/${action.report_name}`;
                if (action.context.active_ids) {
                    reportUrl += "/" + action.context.active_ids.join(",");
                }
