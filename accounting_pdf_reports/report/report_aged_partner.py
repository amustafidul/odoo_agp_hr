    ...
    def _get_report_values(self, docids, data=None):
        ...
        movelines, total, dummy = self._get_partner_move_lines(account_type,
                                                               ...
                                                               data['form'][
                                                                   'period_length'])
        for i in range(5):
            datas = data['form'][str(i)]['name'].split('-')
            # Memeriksa apakah terdapat data kedua (datas[1]) di dalam list datas
            if data['form']['period_length'] != 30:
                if len(datas) > 1:
                    # Menghitung rentang bulan
                    range_month0 = int(int(datas[0]) / 30)
                    if datas[0] == '0':
                        datas[0] = str(range_month0)
                    else:
                        datas[0] = str(range_month0+1)
                    range_month = int(int(datas[1]) / 30)
                    datas[1] = str(range_month) + ' bulan'
                    hasil = f"{datas[0]}-{datas[1]}"
                    data['form'][str(i)]['name'] = hasil
                    # print(datas[0], datas[1])
                else :
                    range_month = int(int(datas[0]) / 30)
                    datas[0] = "+" + str(range_month) + ' bulan'
                    hasil = f"{datas[0]}"
                    data['form'][str(i)]['name'] = hasil 
            else :
                if len(datas) > 1:
